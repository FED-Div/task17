/* 数据格式演示
var aqiSourceData = {
  "北京": {
    "2016-01-01": 10,
    "2016-01-02": 10,
    "2016-01-03": 10,
    "2016-01-04": 10
  }
};
*/

// 以下两个函数用于随机模拟生成测试数据
function getDateStr(dat) {
  var y = dat.getFullYear();
  var m = dat.getMonth() + 1;
  m = m < 10 ? '0' + m : m;
  var d = dat.getDate();
  d = d < 10 ? '0' + d : d;
  return y + '-' + m + '-' + d;
}
function randomBuildData(seed) {
  var returnData = {};
  var dat = new Date("2016-01-01");
  var datStr = ''
  for (var i = 1; i < 92; i++) {
    datStr = getDateStr(dat);
    returnData[datStr] = Math.ceil(Math.random() * seed);
    dat.setDate(dat.getDate() + 1);
  }
  return returnData;
}

var aqiSourceData = {
  "北京": randomBuildData(500),
  "上海": randomBuildData(300),
  "广州": randomBuildData(200),
  "深圳": randomBuildData(100),
  "成都": randomBuildData(300),
  "西安": randomBuildData(500),
  "福州": randomBuildData(100),
  "厦门": randomBuildData(100),
  "沈阳": randomBuildData(500)
};
var city = Object.keys(aqiSourceData);
// 用于渲染图表的数据
var chartData = {};

// 记录当前页面的表单选项
var pageState = {
  nowSelectCity: 1,
  nowGraTime: "day"
}

/**
 * 渲染图表
 */
function renderChart() {
  var html = '';
  for (data in chartData) {
    if (chartData.hasOwnProperty(data)) {
      html += '<div class="data-rect data-'+ pageState.nowGraTime +'" title=' + chartData[data] + ' style="height:'+ chartData[data] +'px"><span>time: ' + data +' <br/> data:' + chartData[data] + ' </span></div>';
    }
  }
  var chart = document.getElementsByClassName('aqi-chart-wrap')[0];
  chart.innerHTML = html;

  // 处理渲染好的数据，添加popup
  renderRect();
}

function renderRect() {
  var rect = document.getElementsByClassName('data-rect');
  for (var i = 0; i < rect.length; i++) {
    rect[i].addEventListener('mouseover', function () {
      this.getElementsByTagName('span')[0].style.opacity = 1;
    });
    rect[i].addEventListener('mouseout', function () {
      this.getElementsByTagName('span')[0].style.opacity = 0;
    })
  }
}
/**
 * 日、周、月的radio事件点击时的处理函数
 */
function graTimeChange() {
  // 确定是否选项发生了变化 
  if ((this.value === pageState.nowGraTime) && (document.getElementsByClassName('data-rect').length !== 0)) {
    return;
  } else {
    pageState.nowGraTime = this.value;
  }
  // 设置对应数据
  initAqiChartData();
  // 调用图表渲染函数
  renderChart();
}

/**
 * select发生变化时的处理函数
 */
function citySelectChange() {
  // 确定是否选项发生了变化 
  if (this.value === pageState.nowSelectCity) {
    console.log( this.value );
    console.info(pageState.nowSelectCity);
    return;
  } else {
    console.log( this.value );
    console.info(pageState.nowSelectCity);
    pageState.nowSelectCity = this.value;
    console.info(pageState.nowSelectCity);  
  }
  // 设置对应数据
  initAqiChartData();
  // 调用图表渲染函数
  renderChart();
}

/**
 * 初始化日、周、月的radio事件，当点击时，调用函数graTimeChange
 */
function initGraTimeForm() {
  var radios = document.getElementsByTagName('input');

  for (var i = 0; i < radios.length; i++) {
    radios[i].addEventListener('click',graTimeChange);
  }
  radios[0].click();
}

/**
 * 初始化城市Select下拉选择框中的选项
 */
function initCitySelector() {
  // 读取aqiSourceData中的城市，然后设置id为city-select的下拉列表中的选项
  var select = document.getElementById("city-select");
  var html = '';
  city.forEach( function(element, index) {
    index += 1;
    html += '<option value="'+ index +'">' + element + '</option>'
  });
  select.innerHTML = html;
  // 给select设置事件，当选项发生变化时调用函数citySelectChange
  select.addEventListener('click', citySelectChange);
}

/**
 * 初始化图表需要的数据格式
 */
function initAqiChartData() {
  // 将原始的源数据处理成图表需要的数据格式
  // 处理好的数据存到 chartData 中
  chartData = aqiSourceData[city[pageState.nowSelectCity-1]];
  switch(pageState.nowGraTime) {
    case "day":
      // chartData = aqiSourceData[city[pageState.nowSelectCity]];
      break;
    case "week":
      var sum = 0;
      var counter = 0;
      var result = [];
      for (var data in chartData) {
        if (chartData.hasOwnProperty(data)) {
          var d = new Date(data).getDay();
          sum += chartData[data];
          counter++;
          if (d == 6) {
            result.push(sum/counter);
            sum = 0;
            counter = 0;
          }
        }
      }
      chartData = result;
      result = null;
      break;
    case "month":
      var sum = 0;
      var counter = 0;
      var result = [];
      var oldMonth = 0;
      for (var data in chartData) {
        if (chartData.hasOwnProperty(data)) {
          counter++;
          sum += chartData[data];
          var nowMonth = new Date(data).getMonth(); //0
          if (nowMonth !== oldMonth) { //ture
            oldMonth = nowMonth; //old = 0
            result.push(sum/counter);
            sum = 0;
            counter = 0;
          }
        }
      }
      if (counter !== 0) {
        oldMonth = nowMonth; //old = 0
        result.push(sum/counter);
        sum = 0;
        counter = 0;
      }
      chartData = result;
      result = null;
      break;
  }
}

/**
 * 初始化函数
 */
function init() {
  initGraTimeForm()
  initCitySelector();
  initAqiChartData();
}

// init();
window.onload = init;
